...
bool InitTest( char*  filePath, FILE *&video )
{
	
	if (fopen_s(&video, filePath, "rb") != 0)
	{
		printf("Could not open video file!\n");
		return false;
	}
	...
	if (ret == 1)
	{
		...
		if (ret == 1)
		{
			...
			if (ret <= len)
			{
				...
				return encoder.MP4AddH264Track(buf, len, 928, 576);
			}
			else
...
{
	char filePath[256] = { "D:\\video\\mp4v2\\bin\\20220510060853videotest.h264" };
	MP4Encoder encoder;
	FILE *video(NULL), *audio(NULL);

	InitTest(filePath, video);

	encoder.MP4CreateFile("D:\\test.mp4", 5);

	((int)AddH264Track(encoder, video));
